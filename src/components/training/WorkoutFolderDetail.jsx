...
// Per-(exercise_id, drill_index) numeric trend graph — the primary
// view at the bottom of the folder. Pulls every exercise_set_logs row
// for this plan's executions in one round-trip, groups by
// (exercise_id, drill_index) so nested inner exercises (super_set /
// circuit / combo / tabata) appear as independent series, and lets the
// trainee pick a metric (sum reps / max reps / volume / seconds) with
// a smart per-series default. Recharts (consistent with the rest of
// this file); the dual reps+weight rendering puts the weight digits
// above each weight point so the load progression is readable at a
// glance without a tooltip hover.
function ExerciseNumericTrendGraph({ plan, completed }) {
  ...
  // Lookup: plan exercise by id — used to resolve names + tabata_data
  // for the nested-inner label below.
  const planExerciseById = useMemo(() => {
    const out = {};
    for (const section of plan?.sections || []) {
      for (const ex of section?.exercises || []) {
  ...
  // Bridge map: execution_id → executed_at. The embedded select also
  // carries executed_at, but legacy rows missing the join can still
  // resolve their date through this fallback.
  const execAtById = useMemo(() => {
    const m = {};
    for (const e of completed || []) m[e.id] = e.executed_at;
    return m;
  }, [completed]);
  ...
  // grouping below can split inner exercises into their own series.
  const { data: setLogRows = [] } = useQuery({
  ...
  const grouped = useMemo(() => {
    ...
    for (const row of setLogRows) {
      ...
      const at = row.workout_executions?.executed_at || execAtById[execId];
  ...
  const seriesList = useMemo(() => {
    ...
    for (const key of Object.keys(grouped)) {
      ...
      const ex = planExerciseById[exerciseId] || {};
    ...
    <div style={{
      ...
      <div style={{ marginBottom: 14, paddingInline: 4 }}>
        ...
        <div style={{
          ...
          מגמת ערכים לאורך זמן · תרגיל יחיד או פנימי
        </div>
      </div>
      ...
      {seriesList.length === 0 ? (
        <div style={{
          ...
          עדיין אין נתונים — בצע אימון כדי לראות מגמה
        </div>
      ) : (
        <>
