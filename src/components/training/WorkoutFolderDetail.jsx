...
// Per-(exercise_id, drill_index) numeric trend graph — folder-scoped
// wrapper around the shared SharedExerciseNumericTrendGraph component.
// The shared component is generic over its data source; this wrapper
// runs the plan-scoped exercise_set_logs query and builds the name+
// tabata_data lookup from the in-memory plan tree so the folder view
// behaves identically to the pre-extraction inline version.
function ExerciseNumericTrendGraph({ plan, completed }) {
  const planId = plan?.id;

  // ONE query for the whole plan: PostgREST embedded select joins
  // workout_executions so we can filter by plan_id server-side without
  // a second round-trip. drill_index is part of the projection so the
  // grouping inside the shared component can split inner exercises
  // into their own series.
  const { data: planSetLogs = [] } = useQuery({
    queryKey: ['plan-set-logs', planId],
    enabled: !!planId,
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select(`
          ...
          workout_executions!inner(executed_at, plan_id)
        `)
  ...
  // Lookup: plan exercise by id — used by the shared chart to resolve
  // names + tabata_data for nested-inner labelling. Built from the
  // already-loaded plan tree so this never costs a round-trip.
  const exerciseById = useMemo(() => {
    const out = {};
    for (const section of plan?.sections || []) {
      for (const ex of section?.exercises || []) {
        if (ex?.id) out[ex.id] = ex;
      }
    }
    return out;
  }, [plan]);

  // Legacy fallback — when a row's PostgREST join is missing
  // executed_at (older logs predating the !inner constraint), patch it
  // from the parent's completed-executions list. This preserves the
  // pre-extraction behaviour where the inline component carried an
  // execAtById bridge for legacy rows.
  const patchedRows = useMemo(() => {
    if (!Array.isArray(planSetLogs) || planSetLogs.length === 0) return planSetLogs;
    if (!Array.isArray(completed) || completed.length === 0) return planSetLogs;
    const execAt = {};
    for (const e of completed) if (e?.id) execAt[e.id] = e.executed_at;
    return planSetLogs.map((row) => {
      if (row?.workout_executions?.executed_at) return row;
      const at = execAt[row?.execution_id];
      if (!at) return row;
      return {
        ...row,
        workout_executions: { ...(row.workout_executions || {}), executed_at: at },
      };
    });
  }, [planSetLogs, completed]);

  return (
    <SharedExerciseNumericTrendGraph
      rows={patchedRows}
...
// Single unified progress graph — kept around as the previous primary
// view (completion% / sets / control / challenge / difficulty based on
// workout_executions.exercise_summaries). The numeric trend above is
// now the primary view, but this component stays in the file so its
// metric/level breakdown logic isn't lost.
