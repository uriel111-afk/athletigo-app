...
function fmtSec(secs) {
  if (secs === null || secs === undefined || secs <= 0) return '0';
...
export default function FloatingClockBar() {
  ...
  const { tabata, pauseTabata, resetTabata } = useActiveTimer();
  ...
  // Determine what to show: Tabata from ActiveTimerContext OR Timer/Stopwatch from ClockContext
  const tabataActive = tabata.running || tabata.screen === 'countdown' || tabata.screen === 'running';
  const clockActive = clock?.activeClock && clock.phase !== 'idle' && clock.phase !== 'done';

  // Don't show on clocks page
  if (onClocksPage) return null;
  // Don't show if nothing is running
  if (!tabataActive && !clockActive) return null;

  // Tabata takes priority
  if (tabataActive) {
    const bg = '#FF6F20';
    const info = tabata.screen === 'running'
      ? `סיבוב ${tabata.currentRound} • סט ${tabata.currentSet}`
      : '';
    ...
    const onTouchMove = (e) => {
      if (!dragging) return; movedRef.current = true;
      const t = e.touches[0];
      setPos({ x: Math.max(0, Math.min(window.innerWidth - 170, t.clientX - startRef.current.x)), y: Math.max(60, Math.min(window.innerHeight - 120, t.clientY - startRef.current.y)) });
    };
    ...
    return (
      <div onTouchStart={onTouchStart} onTouchMove={onTouchMove} onTouchEnd={onTouchEnd} onClick={handleTap}
        style={{ position: 'fixed', left: pos.x, bottom: pos.y !== null ? 'auto' : 90, top: pos.y !== null ? pos.y : 'auto',
          zIndex: 500, background: bg, borderRadius: 20, padding: '12px 18px', minWidth: 160,
          boxShadow: '0 6px 20px rgba(0,0,0,0.35)', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4,
          cursor: 'pointer', direction: 'rtl', userSelect: 'none', transition: dragging ? 'none' : 'left 0.1s, top 0.1s, bottom 0.1s' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <Zap style={{ width: 16, height: 16, color: 'rgba(255,255,255,0.9)' }} />
        ...
        <div style={{ fontSize: 48, fontWeight: 900, color: '#FFF', lineHeight: 1, fontVariantNumeric: 'tabular-nums', letterSpacing: -2, fontFamily: "'Barlow Condensed', system-ui" }}>
          {fmtSec(tabata.timeLeft)}
        </div>
        {info && <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.8)', fontWeight: 600, fontFamily: "'Heebo', sans-serif" }}>{info}</div>}
        <div style={{ display: 'flex', gap: 8, marginTop: 4 }} onClick={e => e.stopPropagation()}>
          <button onClick={pauseTabata} style={{ width: 36, height: 36, borderRadius: '50%', background: '#FFF', border: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
            {tabata.running ? <Pause style={{ width: 16, height: 16, color: bg }} /> : <Play style={{ width: 16, height: 16, color: bg }} />}
          </button>
          <button onClick={resetTabata} style={{ width: 36, height: 36, borderRadius: '50%', background: 'rgba(255,255,255,0.25)', border: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
  ...
  // Timer/Stopwatch from ClockContext (existing logic)
  const Icon = clock.activeClock === 'timer' ? Timer : Clock;
  const bg = '#FF6F20';
  ...
  const onTouchEnd = () => setDragging(false);
  const handleTap = () => { if (!movedRef.current) { if (clock.maximize) clock.maximize(); navigate(createPageUrl('Clocks')); } };
